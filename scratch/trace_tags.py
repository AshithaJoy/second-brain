...
for match in tag_pattern.finditer(cleaned_text):
    ...
    if tag_str.startswith('</'):
        if not stack:
            print(f'L{line_num}: Extra closing </{tag_name}>')
            continue
        top_name, top_line = stack.pop()
        print(f"L{line_num}: Pop </{tag_name}> matching <{top_name}> from L{top_line}. Stack: {[s[0] for s in stack]}")
        ...
        print(f"L{line_num}: Push <{tag_name}>. Stack: {[s[0] for s in stack]}")
